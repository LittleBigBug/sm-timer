...
stock bool LoadZoneFromKv( KeyValues kv )
{
    ...
    kv.GetString( "name", zone.szName, sizeof( Zone_t::szName ), "" );
    if ( zone.szName[0] == 0 )
    {
        // Fallback to section name.
        if ( !kv.GetSectionName( zone.szName, sizeof( Zone_t::szName ) ) )
        {
...
stock void BuildZoneKvs( ArrayList kvs )
{
    ...
    for ( int i = 0; i < g_hZones.Length; i++ )
    {
        ...
        int zoneid = zone.iZoneId;
        if ( zoneid < 1 ) continue;
        
        
        ZoneType_t zonetype = view_as<ZoneType_t>( zone.iZoneType );
        ...
        if ( res == Plugin_Stop )
        {
            LogError( INF_CON_PRE..."Couldn't save zone %s (id: %i) with type %i!", zone.szName, zoneid, zonetype );
