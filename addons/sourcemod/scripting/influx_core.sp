...
stock void BuildRunKvs( ArrayList kvs )
{
    ...
    for ( int i = 0; i < g_hRuns.Length; i++ )
    {
        ...
        FormatEx( szKeyValueName, sizeof( szKeyValueName ), "run%i", run.iId );
        ...
        {
            LogError( INF_CON_PRE..."Run of id %i had invalid teleport location. No location will be saved.", run.iId );
        }

        float teleyaw = view_as<float>( run.flTeleYaw );
        if ( Inf_IsValidTeleAngle( teleyaw ) )
        {
            kv.SetFloat( "teleyaw", run.flTeleYaw );
        }
        else
        {
            LogError( INF_CON_PRE..."Run of id %i had invalid teleport yaw. No yaw will be saved.", run.iId );
        }

        kv.SetString( "name", run.szName );
